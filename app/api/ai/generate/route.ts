...
export async function POST(req: NextRequest) {
  ...
  if (body.userId) {
    const resume = await prisma.resumeVersion.create({ data: { userId: body.userId, title: `Resume for ${body.job?.title || 'job'}`, content: pack.resume || '', language: body.language || 'English', market: body.market || 'EU', score: { atsScore: pack.atsScore, localeScore: pack.localeScore, truthRisk: pack.truthRisk }, suggestions: pack.suggestions || [] } });
    await prisma.coverLetterVersion.create({ data: { userId: body.userId, title: `Cover letter for ${body.job?.title || 'job'}`, content: pack.coverLetter || '', language: body.language || 'English', tone: body.tone || 'professional' } });
    await prisma.historyItem.create({ data: { userId: body.userId, type: 'resume', title: resume.title, details: `ATS ${pack.atsScore || 'n/a'}` } });
  }
