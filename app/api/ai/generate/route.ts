...
BASELINE=${JSON.stringify(deterministicPack)}`;
  ...
  const finalPack = { ...deterministicPack, ...pack, coverLetter: coverLetterEnabled ? (pack.coverLetter || deterministicPack.coverLetter) : '' };
  ...
  let cover = null;
  if (coverLetterEnabled) cover = await prisma.coverLetterVersion.create({ data: { userId, jobPostingId: body.jobPostingId || null, title: `Cover letter for ${body.job?.title || 'job'}`, content: finalPack.coverLetter || deterministicPack.coverLetter, language, tone: body.tone || 'professional' } });
  await prisma.historyItem.create({ data: { userId, type: 'resume', title: resume.title, details: `ATS ${finalPack.atsScore || 'n/a'} · ${language} · ${market} · ${noLiesMode ? 'No Lies ON' : 'No Lies OFF'}`, payload: { resumeVersionId: resume.id, coverLetterVersionId: cover?.id || null, jobPostingId: body.jobPostingId || null, pack: finalPack, noLiesMode, generator } } });
