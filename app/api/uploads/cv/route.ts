...
async function persistParsedProfile(userId: string, parsedProfile: any, sourceName: string, extractedText: string, mimeType: string) {
  ...
  const profile = await prisma.careerProfile.upsert({
    ...
    create: {
      ...
      title: parsedProfile.title || null,
  ...
  if (education.length) {
    await prisma.educationItem.createMany({
      data: education.map((edu: any) => ({
        profileId: profile.id,
        school: edu.school || 'Education from CV',
  ...
  if (projects.length) {
    await prisma.project.createMany({
      data: projects.map((project: any) => ({
        profileId: profile.id,
        name: project.name || 'Project from CV',
        summary: project.summary || null,
        skills: project.skills || [],
        evidence: project.evidence || ['Source: uploaded CV'],
      })),
    });
  }
  ...
  if (certifications.length) {
    await prisma.certification.createMany({
      data: certifications.map((cert: any) => ({
        profileId: profile.id,
        name: cert.name || 'Certification from CV',
