...
export async function POST(req: NextRequest) {
  ...
  return new NextResponse(buffer, { headers: { 'content-type': format === 'pdf' ? 'application/pdf' : 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'content-disposition': `attachment; filename="${title || 'export'}.${format}"` } });
}
