...
export default function VacancyPage() {
  ...
  async function parse() {
    ...
    setMsg('');
    setErr('');
    const res = await fetch('/api/vacancy', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: vacancyName.trim(), company: companyName.trim(), url: url.trim(), text, language: cvLanguage, targetMarket, coverLetterEnabled, coverLetterLanguage, interviewPrep }) });
    const data = await safeJson(res);
    setBusy(false);
    if (!res.ok) { setErr(data.error || 'Could not analyze vacancy. Paste a real job description.'); return; }
    setVacancyName(String(data.analysis?.title || vacancyName || '').trim());
    setCompanyName(String(data.analysis?.company || companyName || '').trim());
    setMsg(t.success);
  }
  ...
  return (
    <AppShell>
      ...
      <div className="card mt-8 p-6">
        ...
        <label className="mt-4 block text-xs font-bold uppercase tracking-widest text-slate-400">{t.company}</label>
        <input className="input mt-2" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder={t.companyPlaceholder} />
